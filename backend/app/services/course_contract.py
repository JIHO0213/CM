...
from app.services import query_router, kakao, hours, reviews
...
def _to_frontend_course(course: dict, index: int, requirement_keywords: list[str]) -> dict:
    places_out = []
...
async def _generate_courses(parsed: dict, exclude_place_names: list[str] | None = None) -> dict:
    ...
    return {
        ...
        "courses": [
            _to_frontend_course(c, i, requirement_keywords) for i, c in enumerate(courses)
        ],
    }


async def _generate_courses_stream(parsed: dict):
    """
    ...
    async def run():
        try:
            return await agents.plan_courses(
                constraints, anchor, seed_anchor_as_place=False, on_progress=on_progress,
            )
        finally:
...
async def handle_free_query(query: str) -> dict:
    parsed = query_router.parse_free_query(query)
    return await _generate_courses(parsed)


async def handle_free_query_stream(query: str):
    parsed = query_router.parse_free_query(query)
    async for chunk in _generate_courses_stream(parsed):
        yield chunk
